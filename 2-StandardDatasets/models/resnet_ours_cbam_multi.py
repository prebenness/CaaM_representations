...
class ResNet(nn.Module):
    def __init__(self, block, layers,  network_type, num_classes, att_type=None, split_layer=4):
        ...
        if network_type == "ImageNet":
            self.conv1 = nn.Conv2d(cfg.img_shape[0], 64, kernel_size=7, stride=2, padding=3, bias=False)
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
            self.avgpool = nn.AvgPool2d(7)
        else:
            self.conv1 = nn.Conv2d(cfg.img_shape[0], 64, kernel_size=3, stride=1, padding=1, bias=False)
